...
with tab1:
    ...
    if model_ready:
        ...
        # 為了保證有些沒列在選單上的罕見特徵名稱也能湊滿 71 欄，補齊剩下的衍生欄位
        existing_count = len(expected_features)
        if existing_count < 71:
            for i in range(71 - existing_count):
                expected_features.append(f"reserved_feature_{i}")

        # 1. 初始化一個全為 0、長度為 71 的 DataFrame
...
with tab3:
    ...
    try:
        ...
        st.error("❌ 找不到原始資料檔案 'zoo_f4.csv'，請確保它跟 app.py 在同一個資料夾。")
        shelter_counts = {}
...
# ==========================================
with tab5:
    #st.markdown('<h1 class="main-title">📊 專題緣起</h1>', unsafe_allow_html=True)
    #st.markdown('<p class="sub-title">流浪動物的未來</p>', unsafe_allow_html=True)
